        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-amber-950 flex items-center">
              <UserCheck className="h-6 w-6 text-amber-600 mr-2" />
              My Requests
            </h1>
            <p className="text-amber-600 mt-1">Track your requests to join games ({pendingRequests.length} pending)</p>
          </div>
          <Button
        ...
        {loading ? (
          <Card className="border border-amber-200 shadow-sm">
            ...
            <CardContent>
              <div className="space-y-4">
                {[...Array(5)].map((_, i) => (
                  <div key={i} className="flex items-center justify-between p-4 border border-amber-200 rounded-lg">
          ...
          <>
            ...
            <Card className="border border-amber-200 shadow-sm mb-6">
              <CardHeader>
                <CardTitle className="text-amber-950 flex items-center">
              ...
              <CardContent>
                {pendingRequests.length === 0 ? (
                  <div className="text-center py-8">
                    <UserCheck className="w-12 h-12 text-amber-200 mx-auto mb-3" />
                    <h3 className="text-lg font-medium text-amber-950 mb-2">No Pending Requests</h3>
                    <p className="text-amber-600">You don't have any pending requests to join games.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {pendingRequests.map((request) => (
                      <div
                        key={request.id}
                        className="flex items-center justify-between p-4 border border-amber-200 rounded-lg hover:bg-amber-100 transition-colors"
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <span className="text-amber-600">You requested to join</span>
                            <div className="flex items-center text-amber-950 font-medium">
                              <Gamepad2 className="h-4 w-4 text-amber-600 mr-1" />
                              {request.game.game}
                            </div>
                          </div>
                        ...
                        <div className="flex gap-2 ml-4">
                          <Button
                            ...
                            size="sm"
                            variant="outline"
                            className="border-amber-200 text-amber-700 hover:bg-red-100 hover:text-red-600 hover:border-red-200"
                          >
                  ...
                  <div className="space-y-3">
                    {approvedRequests.map((request) => (
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <span className="text-amber-600">You were approved to join</span>
                            <div className="flex items-center text-amber-950 font-medium">
                              <Gamepad2 className="h-4 w-4 text-amber-600 mr-1" />
                              {request.game.game}
                            </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {otherRequests.map((request) => (
                      <div
                        key={request.id}
                        className="flex items-center justify-between p-4 border border-amber-200 rounded-lg bg-amber-100 opacity-75"
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <span className="text-amber-600">You requested to join</span>
            ...
            {requests.length === 0 && !loading && (
              <div className="text-center py-16 bg-white rounded-lg border border-amber-200 shadow-sm">
                <UserCheck className="w-16 h-16 text-amber-200 mx-auto mb-4" />
                <h2 className="text-xl font-bold text-amber-950 mb-2">No Requests Found</h2>
